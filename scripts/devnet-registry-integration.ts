...
import { createStrategy, getStrategy, publishRebalance } from "../apps/api/src/repository";
import { hashStrategyAllocation, SUPPORTED_TOKENIZED_EQUITIES, USDC_MINT, type StrategyAllocationDto } from "../packages/shared/src";
import type { PriceProvider } from "../packages/strategy-engine/src";

const PROGRAM_ID = new anchor.web3.PublicKey("2zDw6KYfMJRMVfNvShy5XHM1t6tvvTYEoDeLZ87VTEFX");
const DEVNET_RPC_URL = "https://api.devnet.solana.com";
const DEPLOYER_KEYPAIR_PATH = "/Users/ashrafsyed/Documents/stiNBhrncyQLCXGGZWUYoxizjAH8b6JA8KCZpNrPtGm.json";
...
function assetMint(symbol: string) {
  const asset = SUPPORTED_TOKENIZED_EQUITIES.find((item) => item.symbol === symbol);
  if (!asset) {
...
function loadProgram() {
  const secret = JSON.parse(readFileSync(DEPLOYER_KEYPAIR_PATH, "utf8")) as number[];
  ...
  const idl = JSON.parse(readFileSync(resolve(REPO_ROOT, "target/idl/strategy_registry.json"), "utf8")) as anchor.Idl;
  return {
    creator,
    connection,
...
async function main() {
  ...
  const chainStrategy = await program.account.strategyAccount.fetch(registryStrategyPda);
  const chainVersionOne = await program.account.strategyVersionAccount.fetch(registryVersionOnePda);
  const chainVersionTwo = await program.account.strategyVersionAccount.fetch(registryVersionTwoPda);
  ...
  try {
    await program.methods
      ...
      .signers([stranger])
      .rpc();
    unauthorizedResult = { rejected: false, message: "Unexpectedly accepted unauthorized rebalance." };
  } catch (error) {
    unauthorizedResult = { rejected: true, message: String(error).slice(0, 500) };
  }
  const chainStrategyAfterUnauthorized = await program.account.strategyAccount.fetch(registryStrategyPda);
    ...
    JSON.stringify(
      {
        ...
        unauthorizedResult,
        onChainCurrentVersionAfterUnauthorized: chainStrategyAfterUnauthorized.currentVersion,
