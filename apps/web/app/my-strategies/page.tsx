    ...
    <main className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8">
      ...
      {!walletAddress && hasMounted ? (
        <p className="mt-6 text-amber-200">
          Connect a wallet to view strategies.
        </p>
      ) : null}
      {isLoading ? (
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex gap-2">
                ...
                <Link
                  className="rounded-md border border-teal-300/40 px-3 py-2 text-sm text-teal-100 hover:bg-teal-300/10"
                  href={`/manage/${strategy.id}`}
                >
                  Manage
                </Link>
              </div>
            ...
            <div className="mt-5 grid gap-3 text-sm text-slate-300 sm:grid-cols-4">
              <p>
                Investors
                <br />
                <span className="text-white">{strategy.investorCount}</span>
              </p>
              <p>
                Capital Following
                <br />
              ...
              <p>
                ...
                <span className="text-white">
                  {formatAtomic(
                    BigInt(strategy.strategistEarningsUsdcAtomic ?? "0"),
                    6,
                  )}{" "}
                  USDC
                </span>
      ...
      {walletAddress && !isLoading && strategies.length === 0 ? (
        <p className="mt-6 text-slate-300">No strategies created yet.</p>
      ) : null}
    </main>
  );
}
