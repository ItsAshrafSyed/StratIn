...
function parseWalletChain(value: string | undefined): SolanaWalletChain {
  ...
  if (!value.includes(":")) {
    throw new Error(
      "NEXT_PUBLIC_WALLET_CHAIN must be a Wallet Standard chain such as solana:mainnet.",
    );
  }
...
export const appConfig = {
  solanaRpcProxyUrl:
    process.env.NEXT_PUBLIC_SOLANA_RPC_PROXY_URL ?? "/api/solana-rpc",
  solanaRpcUrl:
  ...
  apiBaseUrl:
    process.env.NEXT_PUBLIC_STRATIN_API_URL ?? "http://localhost:8788",
} as const;
