...
import { canonicalizeAllocationForHash, hashStrategyAllocation, SUPPORTED_TOKENIZED_EQUITIES, USDC_MINT } from "@stratin/shared";
...
describe("strategy allocation hashing", () => {
  ...
  it("keeps hash stable when UI ordering changes", async () => {
    ...
    await expect(hashStrategyAllocation(reversed)).resolves.toBe(await hashStrategyAllocation(allocation));
  });
  ...
  it("changes when weight or mint changes", async () => {
    await expect(hashStrategyAllocation([{ ...allocation[0], weightBps: 2000 }, { ...allocation[1], weightBps: 3500 }, allocation[2], allocation[3]])).resolves.not.toBe(
      "06aa8989e781f6b3030a6cfd201f6df017a33f841844109dab558db70caa7362"
    );
  });
});
